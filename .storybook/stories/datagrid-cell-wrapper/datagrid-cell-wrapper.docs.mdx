...
<Meta of={WrapperStories} name="Overview" />
...
```typescript
import { EcDatagridCellWrapperComponent } from '@extrawest/extra-clarity/datagrid-cell-wrapper';
```
...
a label, and the top offset which may be useful for vertical alignment in non-datagrid use cases.
...
<Markdown>{`See at [GitHub](${extraClarityGithubSrc}/datagrid-cell-wrapper/src)`}</Markdown>
