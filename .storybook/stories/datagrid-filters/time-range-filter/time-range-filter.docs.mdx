...
```typescript
import { TimeRangeFilterComponent } from '@extrawest/extra-clarity/datagrid-filters/time-range-filter';
```
```html
...
(as `TimeRangeFilterValue`) and the list of presets used by the filter (as `TimeRangePreset[]`).
