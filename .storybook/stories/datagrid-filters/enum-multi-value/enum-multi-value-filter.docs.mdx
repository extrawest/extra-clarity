...
```typescript
const options: EnumValueFilterOption<string>[] = ['red','green','blue'].map(value => ({ value, label: value }));
...
You may use the `MarkMatchedStringPipe` for that:
...
<Markdown>
  {`See at [GitHub](${extraClarityGithubSrc}/datagrid-filters/src/enum-multi-value-filter)`}
</Markdown>
