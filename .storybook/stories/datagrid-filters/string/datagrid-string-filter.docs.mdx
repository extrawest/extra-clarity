...
import * as StringFilterStories from './datagrid-string-filter.stories';
...
```typescript
import { DatagridStringFilterComponent } from '@extrawest/extra-clarity/datagrid-filters';
```
```html
...
<clr-datagrid>
  ...
  <clr-dg-column>
    Station ID
    <clr-dg-filter>
      <ec-datagrid-string-filter [propertyKey]="'stationId'" />
    </clr-dg-filter>
  </clr-dg-column>
  ...
</clr-datagrid>
...
```html
...
<ec-datagrid-string-filter #filterByName>
...
