...
This component is suitable for filtering datagrids with a single value from an enumerated list of options.
...
Please refer to the [API](#api) section for the details about another inputs.
...
The component's output `(selectionChanged)` emits a new filter state on every selection change.
...
* put a value into the input `[setValue]`
* pass a value into the method `forceSelection(value: E | null)`
