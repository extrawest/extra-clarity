...
2. `[propertyKey]`
    ...
    * For client-driven datagrids it must be equal to the name of a field in the object passed to the `[clrDgItem]`
        input on `<clr-dg-row>` to filter by, i.e. to decide whether a row should be shown or not.
3. `[options]` An array of grouped options `EnumValueFilterOptionGroup<E>` to select from.
    See the following [section](#option-list) for more details.

### Option List

To set the options to select from, you should put an array of `EnumValueFilterOptionGroup<E>` to
the `[options]` input. Each item of this array contains the configuration for a group of options:

```typescript
interface EnumValueFilterOptionGroup<E> {
    label: string;
    expandedByDefault?: boolean;
    // items: Array<EnumValueFilterOption<E>>
    items: Array<{
        value: E;
        label?: string;
...
```typescript
const options: EnumValueFilterOptionGroup<string>[] = [
  {
    label: 'Grayscale'
    items: [
